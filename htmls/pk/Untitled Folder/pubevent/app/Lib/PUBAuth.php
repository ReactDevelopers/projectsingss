...
class PUBAuth {
	...
	function check($username, $password) {
        ...
        if($this->isDevelopmentMode() ) {
        	...
        	$pubnet_id = $this->checkLdapAuthentication($username, $password);
        }
        ...
        if(!$pubnet_id){
        	...
        	return $this->getUserByPubnetId($pubnet_id);
        }
	}

	public function response() {

		return ['status'=>$this->status,'message'=> $this->message, 'user'=> $this->user];
	}
	...
	public function getUserByPubnetId($pid) {
		...
		$this->user = $user = \App\User::where('pubnet_id', $pid)->first();
	 ...
	 */
	function sendEmail($username, $info) {
