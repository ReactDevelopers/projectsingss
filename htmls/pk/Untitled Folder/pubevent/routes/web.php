...
$router->group(['middleware' => ['cros','auth:api']], function () use ($router) {
    ...
    $router->put('event-allow/{id}',['uses'=>'EventController@updateEventAllow', 'middleware'=>'canAccess:event-update[Admin]']);
    ...
    */
    ...
    /**
     * Email Group Template
    */
    $router->get('email-group',['uses'=>'EmailGroupController@index', 'middleware'=>'canAccess:event-group-list[Admin]']);
    $router->post('email-group',['uses'=>'EmailGroupController@store', 'middleware'=>'canAccess:event-group-create[Admin]']);
    $router->delete('email-group/{id}',['uses'=>'EmailGroupController@delete', 'middleware'=>'canAccess:event-group-delete[Admin]']);
    $router->get('email-group/{id}',['uses'=>'EmailGroupController@get', 'middleware'=>'canAccess:event-group-info[Admin]']);
    $router->put('email-group/{id}',['uses'=>'EmailGroupController@update', 'middleware'=>'canAccess:event-group-update[Admin]']);

    $router->get('email-log',['uses'=>'EventController@emailLog', 'middleware'=>'canAccess:event-group-list[Admin]']);
    $router->get('recipient-list',['uses'=>'EmailController@recipientList', 'middleware'=>'canAccess:event-group-list[Admin]']);

    $router->put('config',['uses'=>'UsersController@updateConfig', 'middleware'=>'canAccess:event-group-list[Admin]']);
    $router->get('config',['uses'=>'UsersController@getConfig', 'middleware'=>'canAccess:event-group-list[Admin]']);
