        ...
        if($request->get('export')) {
            ...
            General::downloadExcel(Excel::create('Report - Course Run', function($excel) use ($data) {
                $excel->sheet('Report - Course Run DATA', function($sheet) use ($data)
                {
                    ...
                    $sheet->cell('K1', function($cell) {$cell->setValue('No. of Attendee(s)')->setValignment('center');});
                    $sheet->cell('L1', function($cell) {$cell->setValue('No. of Absentee(s)')->setValignment('center');});
                    $sheet->cell('M1', function($cell) {$cell->setValue('No. of Penalty(ies)')->setValignment('center');});
                    $sheet->cell('N1', function($cell) {$cell->setValue('No. of Failure(s)')->setValignment('center');});
                    $sheet->cell('O1', function($cell) {$cell->setValue('Overall Average (%)')->setValignment('center');});
                    $sheet->cell('P1', function($cell) {$cell->setValue("Trainer's Delivery (%)")->setValignment('center');});
                    $sheet->cell('Q1', function($cell) {$cell->setValue('Content Relevance (%)')->setValignment('center');});
                    $sheet->cell('R1', function($cell) {$cell->setValue('Site Visits (%)')->setValignment('center');});
                    $sheet->cell('S1', function($cell) {$cell->setValue('Facilities (%)')->setValignment('center');});
                    $sheet->cell('T1', function($cell) {$cell->setValue('Admin (%)')->setValignment('center');});
                    $sheet->cell('U1', function($cell) {$cell->setValue('Response Rate (%)')->setValignment('center');});

                    foreach(['A1','B1','C1','D1','E1','F1','G1','H1','I1','J1','K1','L1','M1','N1','O1','P1','Q1','R1','S1','T1', 'U1'] as $c ) {
                    ...
                    $sheet->cell('N1:U1', function($cell) {
                           $cell->setBackground('#00b0f0');
                    });
                    ...
                    foreach ($data as $index => $row) {
                        ...
                        $sheet->setCellValue('K'.$key, $row->no_of_presentee ? $row->no_of_presentee : 0);
                        $sheet->setCellValue('L'.$key, $row->no_of_absantee ? $row->no_of_absantee : 0);

                        if( $row->no_of_absantee > 0 ) {
                            
                            $sheet->cell('L'.$key, function($cell) {
                                $cell->setBackground('#ffc7ce');
                            });
                        }
                        //
                        $sheet->setCellValue('M'.$key, $row->no_of_penalties ? $row->no_of_penalties : 0); // No of Penalty(s)
                        ...
                        $sheet->setCellValue('N'.$key, $row->no_of_failure ? $row->no_of_failure : 0); // No of Failure(s)
                        ...
                        $sheet->cell('O'.$key, $row->overall ? $row->overall/100 : '');
                        $sheet->cell('P'.$key, $row->trainer_delivery ? $row->trainer_delivery/100 : '');
                        $sheet->cell('Q'.$key, $row->content_relevance ? $row->content_relevance/100 : '');
                        $sheet->cell('R'.$key, $row->site_visits ? $row->site_visits/100 : '');
                        $sheet->cell('S'.$key, $row->facilities ? $row->facilities/100 : '');
                        $sheet->cell('T'.$key, $row->admin ? $row->admin/100 : '');
                        $sheet->cell('U'.$key, $row->response_rate ? $row->response_rate/100 : '');

                        $sheet->getStyle('A'.$key.':T'.$key)->getAlignment()->setWrapText(true)->setVertical(\PHPExcel_Style_Alignment::VERTICAL_CENTER);
                    }
