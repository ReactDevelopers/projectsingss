...
{ 
    ...
    public function uploadPlacement(Request $request) {
        ...
        foreach($data as $key=> $row) {
            ...
            $total++;
            $check =  new PlacementDataVerify($placement_data, $course_run_data, $exists_per_ids);
            $result =  $check->run();
    ...
    public function uploadPlacementResult(Request $request) {
        ...
        $path = $request->file('file')->getRealPath();
        //$data = Excel::load($path)->first();
        $data = \App\Lib\ReadExcelFile::getCollection($path,'F');
        ...
        if($validator->fails()) {
            ...
            $code = 422;
                ...
                if($request->status == 'Confirmed'){
                    ...
                    Placement::whereIn('id', $request->get('placement_id'))
                                ->update(['placements.current_status' => $request->status]);
                    $placements = $request->get('placement_id');
                }
    ...
    */
        ...
        if($validator->fails()) {
            ...
            $code = 422;
                ...
                if($request->status == 'Confirmed') {
                    ...
                    $placement_id = $placements->whereNull('p.id')->groupBy('placements.id')->get(['placements.id'])
                    ->pluck('id')->toArray();
                }
    ...
    public function checkConflict(Request $request){
        ...
        $conflicts = $this->confictInstance($placement_ids)
                ->whereNotNull('p.id')
                ->groupBy('p.id')
                ->with(['participants','subordinates'])
                ->get([
        ...
        if($request->get('export')) {
            ...
            General::downloadExcel(\Excel::create('Post Course Run', function($excel) use ($data) {
                $excel->sheet('Post Course Run DATA', function($sheet) use ($data)
                {
                    ...
                    $sheet->cell('F1', function($cell) {$cell->setValue('Failure Reason');});
                    ...
                    foreach ($data as $index => $row) {
                        ...
                        $sheet->cell('F'.$key, $row->failure_reason);
