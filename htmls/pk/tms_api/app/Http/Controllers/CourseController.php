...
{   
    ...
    public function index(Request $request) {
        ...
        // if($request->has('searchdata') && $s = $request->get('searchdata')){
        ...
        if($request->get('export')) {
            ...
            General::DownloadExcel(Excel::create('Course', function($excel) use ($data) {
                $excel->sheet('Course DATA', function($sheet) use ($data)
                {
                    ...
                    $sheet->cell('F1', function($cell) {$cell->setValue('Dept/ Competency Level (if applicable)');});
                    $sheet->cell('G1', function($cell) {$cell->setValue('Assessment Type');});
                    $sheet->cell('H1', function($cell) {$cell->setValue('Mandatory Y/N');});
                    $sheet->cell('I1', function($cell) {$cell->setValue('Delivery Method');});
                    $sheet->cell('J1', function($cell) {$cell->setValue('Training Location');});
                    $sheet->cell('K1', function($cell) {$cell->setValue('Course Provider');});
                    $sheet->cell('L1', function($cell) {$cell->setValue('Cost per Pax');});
                    $sheet->cell('M1', function($cell) {$cell->setValue('Grant/Subsidy (Y/N)');});
                    $sheet->cell('N1', function($cell) {$cell->setValue('if yes, provide value ($)');});
                    $sheet->cell('O1', function($cell) {$cell->setValue('Vendor Contact/Email Account');});
                    ...
                    foreach ($data as $index => $row) {
                        ...
                        $sheet->cell('I'.$key, $row->delivery_method);
                        $sheet->cell('J'.$key, $row->location);
                        $sheet->cell('K'.$key, $row->provider_name);
                        $sheet->cell('L'.$key, $row->cost_per_pax);
                        $sheet->cell('M'.$key, $row->subsidy);
                        $sheet->cell('N'.$key, $row->subsidy_value);
                        $sheet->cell('O'.$key, $row->vendor_email);

                        $sheet->getStyle('A'.$key.':O'.$key)->getAlignment()->setWrapText(true)->setVertical(\PHPExcel_Style_Alignment::VERTICAL_CENTER);
                    }
    ...
    public function upload(Request  $request) {
        ...
        while($i <= $no_of_rows ) {
            ...
            if($i ==1) {
                ...
                $header = $sheet->rangeToArray('A'.$i.':O'.$i);
                $header = $header[0];
                ...
                $row = $sheet->rangeToArray('A'.$i.':O'.$i,null, false, false);
                $row = $row[0];
