...
class PlacementResultDataVerify extends DataHelper {
    ...
    protected function afterSuccessCallBack() {        
        ...
        $assessment_results = strtolower(isset($this->data['original']['assessment_results']) ? $this->data['original']['assessment_results'] : '');
        $attendance = strtolower(isset($this->data['original']['attendance']) ? $this->data['original']['attendance'] : '');
        
        if( $assessment_results ) {

            switch ($assessment_results) {
                
                case 'pass':                    
                    $this->data['translate'][$this->cellNameToDbCol['assessment_results']] = 'Pass';
                    break;
                case 'fail':
                    $this->data['translate'][$this->cellNameToDbCol['assessment_results']] = 'Fail';
                    break;
                default:
                    # code...
                    break;
        ...
        if( $attendance ) {
            ...
            switch ($attendance) {
                case 'absent':
                    $this->data['translate'][$this->cellNameToDbCol['attendance']] = 'Absent';
                    break;
                case 'present':
                    $this->data['translate'][$this->cellNameToDbCol['attendance']] = 'Present';
                    break;
