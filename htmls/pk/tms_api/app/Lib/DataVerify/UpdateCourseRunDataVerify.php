...
class UpdateCourseRunDataVerify extends DataHelper {
    ...
    protected function afterSuccessCallBack() {
        ...
        $deconflict = strtolower(isset($this->data['original']['deconflict']) ? $this->data['original']['deconflict'] : '');

        if( $deconflict ) {

            switch ($deconflict) {
                case 'yes':
                    $this->data['translate'][$this->cellNameToDbCol['deconflict']] = 'Yes';
                    break;
                case 'no':
                    $this->data['translate'][$this->cellNameToDbCol['deconflict']] = 'No';
                    break;
                default:
                    # code...
                    break;
