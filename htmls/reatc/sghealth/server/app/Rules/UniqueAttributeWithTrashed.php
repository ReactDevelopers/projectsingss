<?php

namespace App\Rules;

use Illuminate\Contracts\Validation\Rule;
//use Illuminate\Validation\Rules\Unique;
use App\Models\User;

class UniqueAttributeWithTrashed implements Rule
{
    /**
     * Create a new rule instance.
     *
     * @return void
     */
    private $ignoreId = null;
    private $data = null;
    private $attribute = null;
    private $table = null;

    public function __construct($ignore_id = null,$table = null)
    {
        //
        $this->ignoreId = $ignore_id;

        $this->table = $table;
    }

    /**
     * Determine if the validation rule passes.
     *
     * @param  string  $attribute
     * @param  mixed  $value
     * @return bool
     */
    public function passes($attribute, $value)
    {
        $this->attribute = $attribute;
        $data = $this->table::where($attribute, $value)->withTrashed();

        if( $this->ignoreId ){

            $data->where('id', '<>', $this->ignoreId);
        }

        $this->data = $data->first();
        return !$this->data;
    }

    /**
     * Get the validation error message.
     *
     * @return string
     */
    public function message()
    {
        if($this->table == 'App\Models\User'){

            if($this->data->status != 'Active') {
    
                return trans('auth.account_inactive');
            }
            else if($this->data->deleted_at) {
                
                return trans('auth.account_suspended');
                
            }
            else {
                return trans("auth.{$this->attribute}_already_exists");
            }
        }else{
            if($this->data->deleted_at) {
                return trans('validation.already_exist_with_trashed');

            }else{
                return trans('validation.unique');
            }
        }
    }
}
