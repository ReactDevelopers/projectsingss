...
class PermissionRepository {
    ...
    /**
     * Throw exception when login user tries to update the information of the user
     *  who does not an employee of loggged in usrer institute
     */
    public function throwExceptionInstituteNotMatched($tag, $user) {

        if( $this->hasTag($tag) ){

            $matched =  array_intersect($user->institute_ids, \Auth::user()->institute_ids);
            if(!count($matched)) {
                abort(403);
            }
        }
    ...
    /**
     * Throw exception when you try to update other institute information.
     */
    public function throwExceptionUpdatingOtherInstitue($institute_id) {

        if($this->hasTag('only_my_institute')) {

            $institute_ids = \Auth::user()->institute_ids;

            if(!$institute_ids || !in_array($institute_id, $institute_ids) ) {
                abort(403);
            }
        }
    }

    public function throwExceptionUpdatingOtherEvent($event_manager_id) {

        if($this->hasTag('my_event_as_manager') && $event_manager_id != \Auth::id()) {
            abort(403);
        }
    }
}
