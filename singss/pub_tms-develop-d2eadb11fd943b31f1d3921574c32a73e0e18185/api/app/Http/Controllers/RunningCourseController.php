...
use App\Models\RunningCourseDate;
use App\Models\Course;
use DB;

class RunningCourseController extends Controller {
    ...
    {
       $running_courses = RunningCourse::select(
       		'running_courses.id',
       		'courses.category',
       		'courses.title',
       ...
       if($request->has('searchdata') && $request->searchdata ){
            ...
            $running_courses->where(function($q) use($request){
        ...
        switch ($request->sortName) {
            ...
            case 'category':
                $running_courses->orderBy('courses.category',$request->sortOrder);
                break;
            case 'title':
                $running_courses->orderBy('courses.title',$request->sortOrder);
                break; 
            case 'duration':
                $running_courses->orderBy('courses.duration',$request->sortOrder);
                break; 
            case 'target_group':
                $running_courses->orderBy('courses.target_group',$request->sortOrder);
                break;
            case 'available_slots':
                $running_courses->orderBy('running_courses.available_slots',$request->sortOrder);
                break;   
            case 'assessment_end_date':
                $running_courses->orderBy('running_courses.assessment_end_date',$request->sortOrder);
                break;         
        ...
        $running_courses = $running_courses->paginate($request->sizePerPage);
        $this->data = $running_courses;
